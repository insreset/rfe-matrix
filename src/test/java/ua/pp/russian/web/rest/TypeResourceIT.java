...
class TypeResourceIT {
    ...
    public static Type createEntity(EntityManager em) {
        Type type = new Type().name(DEFAULT_NAME).picture(DEFAULT_PICTURE);
        return type;
    }
    ...
    public static Type createUpdatedEntity(EntityManager em) {
        Type type = new Type().name(UPDATED_NAME).picture(UPDATED_PICTURE);
        return type;
    }
    ...
    void putNewType() throws Exception {
        ...
        updatedType.name(UPDATED_NAME).picture(UPDATED_PICTURE);
    ...
    void putWithMissingIdPathParamType() throws Exception {
        ...
        restTypeMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(typeDTO))
            )
    ...
    void partialUpdateTypeWithPatch() throws Exception {
        ...
        partialUpdatedType.name(UPDATED_NAME).picture(UPDATED_PICTURE);
    ...
    void deleteType() throws Exception {
        ...
        // Delete the type
        restTypeMockMvc
            .perform(delete(ENTITY_API_URL_ID, type.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
